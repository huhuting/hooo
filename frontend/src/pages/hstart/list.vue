            ...
            <section class="page-section mb-3" >
                <div class="container-fluid">
                    <div class="grid justify-content-between align-items-center">
                        ...
                        <div  class="col-fixed " >
                            <template v-if="auth.canView('hstart/add')">
                                <router-link :to="`/hstart/add`">
                                    <Button :label="$t('addNewHStart')" icon="pi pi-plus" type="button" class="p-button w-full bg-primary "  />
                                </router-link>
                            </template>
                        </div>
                    ...
                    <div  class="col comp-grid" >
                        ...
                        <div >
                            <template v-if="showBreadcrumbs && $route.query.tag && !isSubPage">
                                <Breadcrumb :home="{icon: 'pi pi-home', to: '/hstart'}" :model="pageBreadCrumb" />
                            </template>
                                    ...
                                    <Column selectionMode="multiple" headerStyle="width: 2rem" />
                                        <Column  field="start_id" :header="$t('startId')" >
                                            <template #body="{ data, index }">
                                                <router-link :to="`/hstart/view/${data.start_id}`">
                                                    {{ data.start_id }}
                                                </router-link>
                                            </template>
                                        </Column>
...
<script setup>
	...
	function getActionMenuModel(data){
		...
		{
			label: () => $t('view'),
			to: `/hstart/view/${data.start_id}`,
			icon: "pi pi-eye",
			visible: auth.canView('hstart/view')
		},
		{
			label: () => $t('edit'),
			to: `/hstart/edit/${data.start_id}`,
			icon: "pi pi-pencil",
			visible: auth.canView('hstart/edit')
		},
		{
			label: () => $t('delete'),
			command: (event) => { deleteItem(data.start_id) },
			icon: "pi pi-trash",
			visible: auth.canView('hstart/delete')
		}
	]
	}
	
	onMounted(()=>{ 
		const pageTitle = $t('hStart');
