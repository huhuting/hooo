            ...
            <section class="page-section mb-3" >
                <div class="container-fluid">
                    <div class="grid justify-content-between align-items-center">
                        ...
                        <div  class="col-fixed " >
                            <template v-if="auth.canView('materialtags/add')">
                                <router-link :to="`/materialtags/add`">
                                    <Button :label="$t('addNewMaterialTag')" icon="pi pi-plus" type="button" class="p-button w-full bg-primary "  />
                                </router-link>
                            </template>
                        </div>
                    ...
                    <div  class="col comp-grid" >
                        ...
                        <div >
                            <template v-if="showBreadcrumbs && $route.query.tag && !isSubPage">
                                <Breadcrumb :home="{icon: 'pi pi-home', to: '/materialtags'}" :model="pageBreadCrumb" />
                            </template>
                                    ...
                                    <Column selectionMode="multiple" headerStyle="width: 2rem" />
                                        <Column  field="material_id" :header="$t('materialId')" >
                                            <template #body="{ data, index }">
                                                <Button class="p-button-text" icon="pi pi-eye" :label="$t('materials')" v-if="data.material_id" @click="app.openPageDialog({ page: 'materials/view', url: `/materials/view/${data.material_id}` , closeBtn: true })" />
                                            </template>
                                        </Column>
                                ...
                                <template v-if="showFooter && pageReady">
                                    <div class="grid justify-content-between align-items-center">
                                        <div class="flex gap-2 flex-grow-0">
                                            <template v-if="auth.canView('materialtags')">
...
<script setup>
	...
	const props = defineProps({
		...
		routeName: {
			type: String,
			default: 'materialtagslist',
		},
		apiPath: {
			type: String,
			default: 'materialtags/index',
		},
		autoLoad: {
			type: Boolean,
			default: true,
	...
	function getActionMenuModel(data){
		...
		{
			label: () => $t('view'),
			to: `/materialtags/view/${data.tag_id}`,
			icon: "pi pi-eye",
			visible: auth.canView('materialtags/view')
		},
		{
			label: () => $t('edit'),
			to: `/materialtags/edit/${data.tag_id}`,
			icon: "pi pi-pencil",
			visible: auth.canView('materialtags/edit')
		},
		{
			label: () => $t('delete'),
			command: (event) => { deleteItem(data.tag_id) },
			icon: "pi pi-trash",
			visible: auth.canView('materialtags/delete')
		}
	]
	}
	
	onMounted(()=>{ 
		const pageTitle = $t('materialTags');
