            ...
            <section class="page-section mb-3" >
                <div class="container-fluid">
                    <div class="grid justify-content-between align-items-center">
                        ...
                        <div  class="col-fixed " >
                            <template v-if="auth.canView('replyreply/add')">
                                <router-link :to="`/replyreply/add`">
                                    <Button :label="$t('addNewReplyReply')" icon="pi pi-plus" type="button" class="p-button w-full bg-primary "  />
                                </router-link>
                            </template>
                        </div>
                    ...
                    <div  class="col comp-grid" >
                        ...
                        <div >
                            <template v-if="showBreadcrumbs && $route.query.tag && !isSubPage">
                                <Breadcrumb :home="{icon: 'pi pi-home', to: '/replyreply'}" :model="pageBreadCrumb" />
                            </template>
                            <!-- page records template -->
                            <div class="page-records"  >
                                <DataTable :lazy="true"   :loading="loading"    v-model:selection="selectedItems"
                                     :value="records" dataKey="replys_id" @sort="onSort($event)" class=" p-datatable-sm" :stripedRows ="true" :showGridlines="false" :rowHover="true" responsiveLayout="stack">
                                    <Column selectionMode="multiple" headerStyle="width: 2rem" />
                                        <Column  field="replys_id" :header="$t('replysId')" >
                                            <template #body="{ data, index }">
                                                <router-link :to="`/replyreply/view/${data.replys_id}`">
                                                    {{ data.replys_id }}
                                                </router-link>
                                            </template>
                                        </Column>
...
<script setup>
	...
	const props = defineProps({
		...
		routeName: {
			type: String,
			default: 'replyreplylist',
		},
		apiPath: {
			type: String,
			default: 'replyreply/index',
		},
		autoLoad: {
			type: Boolean,
			default: true,
	...
	function getActionMenuModel(data){
		...
		{
			label: () => $t('view'),
			to: `/replyreply/view/${data.replys_id}`,
			icon: "pi pi-eye",
			visible: auth.canView('replyreply/view')
		},
		{
			label: () => $t('edit'),
			to: `/replyreply/edit/${data.replys_id}`,
			icon: "pi pi-pencil",
			visible: auth.canView('replyreply/edit')
		},
		{
			label: () => $t('delete'),
			command: (event) => { deleteItem(data.replys_id) },
			icon: "pi pi-trash",
			visible: auth.canView('replyreply/delete')
		}
	]
	}
	
	onMounted(()=>{ 
		const pageTitle = $t('replyReply');
