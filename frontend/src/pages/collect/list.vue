            ...
            <section class="page-section mb-3" >
                <div class="container-fluid">
                    <div class="grid justify-content-between align-items-center">
                        ...
                        <div  class="col-fixed " >
                            <template v-if="auth.canView('collect/add')">
                                <router-link :to="`/collect/add`">
                                    <Button :label="$t('addNewCollect')" icon="pi pi-plus" type="button" class="p-button w-full bg-primary "  />
                                </router-link>
                            </template>
                        </div>
                    ...
                    <div  class="col comp-grid" >
                        ...
                        <div >
                            <template v-if="showBreadcrumbs && $route.query.tag && !isSubPage">
                                <Breadcrumb :home="{icon: 'pi pi-home', to: '/collect'}" :model="pageBreadCrumb" />
                            </template>
                                    ...
                                    <Column selectionMode="multiple" headerStyle="width: 2rem" />
                                        <Column  field="collect_id" :header="$t('collectId')" >
                                            <template #body="{ data, index }">
                                                <router-link :to="`/collect/view/${data.collect_id}`">
                                                    {{ data.collect_id }}
                                                </router-link>
                                            </template>
                                        </Column>
                                ...
                                <template v-if="showFooter && pageReady">
                                    <div class="grid justify-content-between align-items-center">
                                        <div class="flex gap-2 flex-grow-0">
                                            <template v-if="auth.canView('collect')">
...
<script setup>
	...
	function getActionMenuModel(data){
		...
		{
			label: () => $t('view'),
			to: `/collect/view/${data.collect_id}`,
			icon: "pi pi-eye",
			visible: auth.canView('collect/view')
		},
		{
			label: () => $t('edit'),
			to: `/collect/edit/${data.collect_id}`,
			icon: "pi pi-pencil",
			visible: auth.canView('collect/edit')
		},
		{
			label: () => $t('delete'),
			command: (event) => { deleteItem(data.collect_id) },
			icon: "pi pi-trash",
			visible: auth.canView('collect/delete')
		}
	]
	}
	
	onMounted(()=>{ 
		const pageTitle = $t('collect');
