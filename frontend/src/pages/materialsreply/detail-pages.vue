                ...
                <TabPanel>
                    <template #header>
                        <div class=" text-lg font-bold" >
                            {{ $t('materialsReplyReplyReply') }}
                        </div>
                    </template>
                    <div class="card my-3 p-3 surface-50">
                        <ReplyreplyListPage ref="replyreplyListPage"  field-name="reply_reply.reply_id" :field-value="masterRecord.reply_id" :show-header="false" :show-breadcrumbs="false" isSubPage>
                        </ReplyreplyListPage>
                    </div>
                </TabPanel>
            </template>
...
const store = usePageStore('MATERIALSREPLY');
const app = useApp();
const auth = useAuth();
