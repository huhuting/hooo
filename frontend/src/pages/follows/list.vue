                                    ...
                                    <Column selectionMode="multiple" headerStyle="width: 2rem" />
                                        <Column  field="id" :header="$t('id')" >
                                            <template #body="{ data, index }">
                                                <router-link :to="`/follows/view/${data.id}`">
                                                    {{ data.id }}
                                                </router-link>
                                            </template>
                                        </Column>
                                        <Column  field="follower_id" :header="$t('followerId')" >
                                            <template #body="{ data, index }">
                                                <Button class="p-button-text" icon="pi pi-eye" :label="$t('users')" v-if="data.follower_id" @click="app.openPageDialog({ page: 'users/view', url: `/users/view/${data.follower_id}` , closeBtn: true })" />
                                            </template>
                                        </Column>
                                        <Column  field="following_id" :header="$t('followingId')" >
                                            <template #body="{ data, index }">
                                                <Button class="p-button-text" icon="pi pi-eye" :label="$t('users')" v-if="data.following_id" @click="app.openPageDialog({ page: 'users/view', url: `/users/view/${data.following_id}` , closeBtn: true })" />
                                            </template>
                                        </Column>
...
<script setup>
	...
	function getActionMenuModel(data){
		...
		{
			label: () => $t('view'),
			to: `/follows/view/${data.id}`,
			icon: "pi pi-eye",
			visible: auth.canView('follows/view')
		},
		{
			label: () => $t('edit'),
			to: `/follows/edit/${data.id}`,
			icon: "pi pi-pencil",
